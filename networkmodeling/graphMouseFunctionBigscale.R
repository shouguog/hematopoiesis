...
mat_zscores<-function(corrMat){
  z_row<-scale(t(corrMat))**2; #cat(dim(z_row),"\n");
  z_col<-scale(corrMat)**2;    #cat(dim(z_col),"\n");
...
load("../../regNetwork/networkmouse.RData")

correlations<-as.numeric(results.mouse$correlations)
rownames(correlations)<-mouseGeneNameConvert(rownames(correlations))
colnames(correlations)<-mouseGeneNameConvert(colnames(correlations))

grn <- globalGRNFromCorr(correlations, tfs, 3)
colnames(grn)[1:2]<-c("TG", "TF");
library(igraph)
ggrn<- ig_tabToIgraph(grn, simplify = TRUE,weights = TRUE)
x <- list(GRN=ggrn, GRN_table=grn, tfs=tfs)
save(x, file="ResultMouse.RData")
